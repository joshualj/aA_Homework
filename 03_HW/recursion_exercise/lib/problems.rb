...
def sum_array(array)
    ...
    sum = array[0] + array[-1]
    array[0] = sum
    sum_array(array.slice(0, array.length - 1)) #could have just used top line + second line of: array[0] + sum_array(array[1..-1])
end
...
def flatten(data)
    ...
    return data if data.flatten(1) == data
    flatten(data.flatten(1))
end
