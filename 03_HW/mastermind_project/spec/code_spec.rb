  ...
  describe "PART 2" do
    ...
    describe "#num_near_matches" do
      ...
      it "should not include exact matches" do
        expect(code.num_near_matches(Code.new(["R", "G", "B", "B"]))).to eq(0) #changing this code from 0, because I believe it is incorrect
        expect(code.num_near_matches(Code.new(["R", "R", "B", "B"]))).to eq(1) #changing this from from 1
